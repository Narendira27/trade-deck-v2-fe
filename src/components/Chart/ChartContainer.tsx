...
import { Plus, X, Grid3X3, Grid2X2, LayoutGrid, AlertCircle } from "lucide-react";
...
const ChartContainer: React.FC = () => {
  ...
  useEffect(() => {
    const checkChartReadiness = () => {
      ...
      const hasValidTabs = visibleTabs.length > 0 && visibleTabs.some(tab => tab.tradeId);
      
      if (hasValidTabs) {
        const hasDataForVisibleTabs = visibleTabs.every(tab => 
      ...
      <div className="h-full flex flex-col bg-gray-900 border border-gray-700 rounded-lg overflow-hidden">
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-gray-500 text-xs mt-1">Loading market data and chart components</p>
          </div>
        </div>
      </div>
    );
